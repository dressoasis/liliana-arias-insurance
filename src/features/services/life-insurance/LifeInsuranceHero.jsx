...
import { lifeInsuranceContent } from "@/content/services/life-insurance";
...
// ── Life Insurance Hero Block ─────────────────────────────────────────────────
export const LifeInsuranceHero = () => {
  const navigate = useNavigate();
  const { openConsultationModal } = useConsultationModal();
  const { badge, title, description, benefits, highlight, buttons, image } = lifeInsuranceContent;
  ...
  const handlePrimary = () => openConsultationModal("life");
  ...
  return (
    <section
      aria-labelledby="life-insurance-hero-heading"
    ...
    >
      ...
      <div
        ...
        aria-hidden="true"
      >
        <div className="absolute top-0 left-0 w-[600px] h-[500px] bg-[#f0f9ff] rounded-full -translate-x-1/2 -translate-y-1/3 blur-3xl opacity-60" />
        <div className="absolute bottom-0 right-0 w-[400px] h-[400px] bg-[#f0f9ff] rounded-full translate-x-1/3 translate-y-1/4 blur-3xl opacity-40" />
      </div>

      {/* ── Main Grid ── */}
      <div className="relative max-w-[1280px] mx-auto px-6 sm:px-8 lg:px-12 py-20 lg:py-28">
        <div className="grid grid-cols-1 lg:grid-cols-[45%_55%] gap-14 lg:gap-16 xl:gap-20 items-center">

          {/* ── LEFT (Desktop) / BOTTOM (Mobile): Content ── */}
          <div
            ref={textRef}
            className={clsx(
              "flex flex-col gap-8 transition-all duration-500 ease-out order-2 lg:order-1",
          ...
          >
            ...
            {/* Título */}
            <Heading
              id="life-insurance-hero-heading"
              level="h2"
          ...
          <div
            ref={imgRef}
            className={clsx(
              "relative w-full transition-all duration-500 ease-out order-1 lg:order-2",
          ...
          >
            ...
            <div
              className="absolute -bottom-4 -left-4 w-24 h-24 rounded-[20px] bg-[#e0f2fe] -z-10"
              aria-hidden="true"
            />
            <div
              className="absolute -top-4 -right-4 w-16 h-16 rounded-full bg-[#f0f9ff] border border-[#bae6fd]/60 -z-10"
              aria-hidden="true"
            />
          </div>
